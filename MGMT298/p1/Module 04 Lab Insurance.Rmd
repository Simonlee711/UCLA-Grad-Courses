...
```{r}
ggplot(data=??, aes(x=??, y=proportion, fill=sex)) +
  geom_col(width=0.5) +
  geom_errorbar(aes(ymin=proportion-1.96*sqrt(proportion*(1-proportion)/total), 
                    ymax=proportion+1.96*sqrt(proportion*(1-proportion)/total)), width=0.25) +
...
```{r}
regression2 = lm(expenses ~ age + ?? + ?? + ?? + ??, data = INSURANCE)
...
```{r}
regression4 = lm(expenses ~ age + children + bmi + sex + smoker + ?? * ??, data = INSURANCE)
...
predict(regression2, CUSTOMER, interval="predict") 
predict(regression4, CUSTOMER, interval="predict") 
```
